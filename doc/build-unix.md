UNIX BUILD NOTES
====================
Some notes on how to build BSUSD in Unix.

(for OpenBSD specific instructions, see [build-openbsd.md](build-openbsd.md))

Note
---------------------
Always use absolute paths to configure and compile bsusd and the dependencies,
...
This will build bsusd-qt as well if the dependencies are met.
...
memory available when compiling BSUSD. On systems with less, gcc can be
...
See the section "Disable-wallet mode" to build BSUSD without wallet.
...
If you want to build BSUSD-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a bsusd-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip bsusdd" to strip the debug
...
# Configure BSUSD to use our own-built instance of BDB
cd $BITCOIN_ROOT
./autogen.sh
...
Security
--------
To help make your bsusd installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, bsusd should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, bsusd may be compiled in
...
This example lists the steps necessary to setup and build a command line only, non-wallet distribution of the latest changes on Arch Linux:
    ...
    git clone https://github.org/bsusd/.git
    cd bsusd/
    ./autogen.sh
...
As mentioned above, when maintaining portability of the wallet between the standard BSUSD distributions and independently built
