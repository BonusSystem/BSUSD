...
- [Development guidelines](#development-guidelines)
    - [General BSUSD](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BSUSD uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
BSUSD is a multi-threaded application, and deadlocks or other
...
BSUSD can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as BSUSD, where everyone uses
...
pay attention to for reviewers of BSUSD code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: BSUSD uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of BSUSD, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against BSUSD so that they can be integrated
...
file descriptors, it will cause problems with BSUSD's `select()` loop, because
...
compatibility between nodes. This happened in BSUSD 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to BSUSD's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of BSUSD, the wallet was always
